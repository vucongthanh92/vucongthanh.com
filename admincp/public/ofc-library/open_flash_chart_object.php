...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
	...
	$out[] = 'var so = new SWFObject("'. $base .'flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<param name="movie" value="'. $base .'flash-chart.swf?width='. $width .'&height='. $height . '&data='. $url .'" />';
    $out[] = '<param name="quality" value="high" />';
	$out[] = '<param name="menu" value="false" />';
	$out[] = '<param name="wmode" value="transparent" />';
    $out[] = '<param name="bgcolor" value="#FFFFFF" />';
    $out[] = '<embed src="'. $base .'flash-chart.swf?data=' . $url .'" wmode="transparent" menu="false" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
